...
## Phase 4: Static & Centralized Schema Refactoring
- **Architectural Shift:** Refactored the dashboard to eliminate dynamic Gemini LLM schema generation (`/api/orchestrate`) to guarantee instant load times, eliminate API costs, and improve determinism.
- **Centralized Schema Configuration (`dashboard_schema.json`):** Created a single, root-level JSON schema file as the shared source of truth for both the Python backend and Next.js frontend. This file defines spreadsheet ranges, column index type mappings, KPI operations, and chart aggregations.
- **Dynamic Backend Parser (`main.py`):** Updated the FastAPI backend to parse Google Sheets values dynamically using the column schemas in `dashboard_schema.json`, ensuring proper type conversions without hardcoding.
- **Dynamic Frontend Dashboard (`page.tsx`):** Rewrote the Next.js React component to load the JSON schema and dynamically aggregate chart values and compute KPIs (sum, count, average) based on metadata parameters, completely removing custom calculation conditions.
- **Documentation (`dashboard.md`):** Updated the dashboard documentation to explain the new centralized architecture contract and how to easily extend columns, KPIs, filters, and charts.
